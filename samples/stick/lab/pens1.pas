  ...
  begin
    ...
    with it^ do
      begin
        ...
        deleteobject(thebmp);
        theregion:=selectobject(handle,oldregion);
...
procedure setrgn(DC: SDC; Region: hrgn);
  begin
    if (DC <> nil) and (region>0) then
    with DC^ do
      begin
        theregion:=selectobject(handle,region);
        if oldregion=0 then oldregion:=theregion else deleteobject(theregion);
      end;
 end;

function resetrgn(DC:SDC):hrgn;
  begin
    with DC^ do
      begin
        theregion:=selectobject(handle,oldregion);
...
begin
  ...
  repeat
    ...
    r1:=createrectrgn(0,0,500,400);
    r2:=createroundrectrgn(150,150,200,200,10,10);
    rf:=createrectrgn(0,0,1,1);
    combinergn(rf,r1,r2,rgn_xor);
    gotoxy(1,1);write(rf);clreol;
    setrgn(dman,rf);
    deleteobject(r1);
    deleteobject(r2);

    deleteobject(resetrgn(dman));
    ...
    for x:=1 to 55 do
      begin
        ...
        killdc(mman);
        setpen(mman,color[1],0,0);
        setbrush(mman,color[1],0,0);
      end;
    ellipse(dman^.handle,20,20,50,60);
    c:=random(16);
    setbrush(dman,color[c],-1,0);
    box(dman,100,100,400,300,0,0);
    ...
    mman := makedc(0,1);
    bp:=loadbmp('C:\Russ\stick\blank.bmp');
    setbmp(mman,bp);
    killDC(dman);

    setbmp(dman,bp);
    setpen(dman,color[1],0,0);
  until keypressed;
