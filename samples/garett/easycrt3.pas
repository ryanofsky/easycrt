...
const
  ...
  WindowSize: TPoint =                         { CRT window size }
    (X: cw_UseDefault; Y: cw_UseDefault);
  InactiveTitle: PChar = '%s has finished.  Close it!';      { Inactive window title }
...
var
  ...
  MouseLocO, MouseLocF, MouseLoc: points;
...
begin
     ...
     for y:=yy to yy+Min(55,ym) do
          for x:=xx to xx+Min(55,xm) do
              bx2.pts[x-xx,y-yy]:=GetPixel(TheDC,x,y);
end;
...
begin
     for y:=yy to yy+bx2.size.y do
          for x:=xx to xx+bx2.size.x do
              SetPixel(TheDC,x,y,bx2.pts[x-xx,y-yy]);
end;
{/Garett2}
